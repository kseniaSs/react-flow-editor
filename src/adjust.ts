...
//  */
// export const adjust = (prev: Map<string, NodeState>, size: Size, nodes: Node[]): Map<string, NodeState> => {
...
//         isCollapsed:
//           node.initial !== undefined && node.initial.isCollapsed !== undefined ? node.initial.isCollapsed : true,
...
//       if (columnToId.has(item.column)) columnToId.get(item.column).push(item.id)
//       else columnToId.set(item.column, [item.id])
